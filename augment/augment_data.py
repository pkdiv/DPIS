    ...
    for aug_number in [1, 2, 3, 4]:
        ...
        for filename in tqdm(images):
            ...
            if not path.exists(DATAPATH + '/Aug{}'.format(aug_number)):
                #print('dir')
                #os.makedirs(DATAPATH + '/Aug{}/{}'.format(aug_number,filename.split('/')[-1]))
                os.makedirs(DATAPATH + '/Aug{}'.format(aug_number))
            #np.save(DATAPATH+'/Aug{}/{}'.format(aug_number, filename.split('/')[-1]), aug)
            np.save(DATAPATH+'/Aug{}/'.format(aug_number)+filename.split('/')[-1].split('\\')[1], aug)
            #np.save(DATAPATH + '/Aug{}'.format(aug_number), aug)
