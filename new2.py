...
def apply_kernel_at(get_value, kernel, i, j):
    # print(type(kernel),print(get_value))
...
def calculate_angles(im, W, f, g):
    ...
    # print(type(sobel.sobelOperator))
    ySobel = sobel.sobelOperator
    # xSobel = transpose(sobel.sobelOperator)
    numpy_array = np.array(ySobel)
    transpose = numpy_array.T
    xSobel = transpose.tolist()
    # print(type(ySobel),type(ySobel[0]))
    # print(type(xSobel), type(xSobel[0]))
...
def apply_kernel_with_f(pixels, kernel, f):
    size = len(kernel)
    # print(type(pixels),type(kernel),type(f))
    for i in range(int(size / 2), int(len(pixels) - size / 2)):
        for j in range(int(size / 2), int(len(pixels[i]) - size / 2)):
            # print(kernel, i, j)
            pixels[i][j] = f(pixels[i][j], apply_kernel_at(lambda x, y: pixels[int(x)][int(y)], kernel, i, j))
